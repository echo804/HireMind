...
<code class="language-python"># 定义工具的结构（以 OpenAI function calling 格式为例）
...
# 然后你的代码负责真正执行这个搜索，把结果再塞回给 LLM</code>
...
<code class="language-python"># Agent 运行的核心 loop（伪代码）
def agent_run(user_goal: str):
    ...
    for step in plan:
        ...
        if action.type == "tool_call":
            ...
            return action.content  # LLM 判断任务完成，返回最终答案</code>
