...
<code class="language-python">class ShortTermMemory:
    def __init__(self):
...
response = llm.chat(messages=memory.get_context())</code>
...
def retrieve_memory(query: str, top_k: int = 3) -> list[str]:
    ...
    return results["documents"][0]</code>
...
<code class="language-python">def run_agent_with_memory(user_request: str, long_term_memory, short_term_memory):
