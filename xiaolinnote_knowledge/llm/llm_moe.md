...
<code class="language-python"># Router 的核心计算（简化版）
gate_logits = token_embedding @ W_router   # 算每个专家的偏好分数
expert_weights = softmax(gate_logits)      # 归一化成概率
top_k_experts = topk(expert_weights, k=2)  # 选 Top-K 个专家</code>
...
<code class="language-">Dense 模型 70B：
