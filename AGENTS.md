...
- [x] 简历上传（PDF/DOCX）与 AI 解析（使用用户 AI 配置，raw_text 原文存库）
- [x] 简历列表/详情/人工校正（编辑保存）
- [x] **AI 简历诊断与润色**（面试官手册分层评估 + 不合理建议 + 项目真实性判断 + 润色导出 docx/pdf，基于原模板排版，不覆盖原文件）
- [x] 模拟面试（文字版，AI 出题、逐题反馈、SSE 流式输出）
- [x] 面试体验增强：实时评估 + 难度自适应 + 计时器 + 跳过/请求提示/润色回答 + 设置面板（难度/风格）
- [x] **面试回顾本**（历史问题列表 + 抽认卡复习模式）
- [x] 面试复盘报告（逐题点评、维度雷达图、PDF 导出）
- [x] 知识库（文档上传、异步切片、向量化、语义搜索、AI 问答）
- [x] 面试日程（周视图、CRUD）
...
- [x] 首页数据概览 / 欢迎页
- [x] Redis 缓存（简历列表、知识库列表/搜索，user_id 隔离）
- [ ] 语音面试（阿里云百炼 ASR/TTS，InterviewType.VOICE 已预留）
- [ ] SSE 断线重连 / 会话恢复（当前 answer-stream 一次性消费，已做请求级重试）
- [ ] 面试报告导出优化（PDF 中文样式）
- [ ] 知识库文档批量管理 / 去重
