...
### 🩺 AI 简历诊断与润色
内置**面试官评审手册**：对简历进行分层打分（职业素养/硬性匹配/经历质量/技术深度/软素质，大模型岗叠加 5 维加试），指出**不合理之处**并判断**项目是否为 Demo**。一键 AI 润色，基于**原简历模板排版**导出 docx / pdf，全程不覆盖原文件。
...
### 📚 知识库 RAG
上传岗位相关的技术文档、公司资料、行业报告，系统自动切片并向量化存储。面试时 AI 可关联知识库内容进行**深度提问**，模拟真实面试中基于特定领域知识的考察方式。支持**知识库 AI 问答**。

</td>
<td width="50%">

### 📒 面试回顾本
集中复习历史面试中面试官问过的**问题、你的回答与 AI 点评**。支持**列表模式**（按方向分组 + 搜索）与**抽认卡模式**（先看题自答再翻看点评），可跳转原报告。
...
| 状态 | 功能 | 说明 |
|:---:|------|------|
| ✅ | 简历上传与 AI 解析 | PDF/DOCX 多格式支持，实时进度反馈，raw_text 原文存库 |
| ✅ | AI 简历诊断与润色 | 面试官手册分层评估 + 不合理建议 + 项目真实性判断 + 润色导出（基于原模板） |
| ✅ | 模拟面试引擎 | AI 个性化出题 + 多轮追问 + SSE 流式 |
| ✅ | 面试体验增强 | 实时评估 + 难度自适应 + 计时器 + 跳过/提示/润色 + 设置面板 |
| ✅ | 面试回顾本 | 历史问题列表 + 抽认卡复习 |
| ✅ | 多维能力报告 | 雷达图 + 评分 + 逐题点评 + PDF 导出 |
| ✅ | 知识库 RAG + AI 问答 | 文档切片向量化 + 语义检索 + 问答 |
| ✅ | 面试日程 | 周视图日历 CRUD |
| ✅ | 多模型切换 | 百炼 / DeepSeek / OpenAI |
| 🔄 | 语音面试 | ASR 语音输入 + TTS 语音播报（InterviewType.VOICE 已预留） |
| 📅 | SSE 断线重连 / 会话恢复 | 当前 answer-stream 已做请求级重试，需完整恢复 |
| 📅 | 知识库文档批量管理 / 去重 | 多选删除、相似文档查重 |
