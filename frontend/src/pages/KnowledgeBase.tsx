...
import { FileText, FileType2, File as FileIcon } from "lucide-react";
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <label className="px-4 py-2 bg-brand-600 text-white text-sm font-medium rounded-lg hover:bg-brand-700 cursor-pointer transition-colors">
          {uploading ? "上传中..." : "上传文档"}
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 mb-6">
        ...
        {searchResults.length > 0 && (
          <div className="mt-4 space-y-3 border-t border-line pt-4">
            <p className="text-sm font-medium text-ink">搜索结果</p>
            {searchResults.map((r, i) => (
              <div key={i} className="bg-surface-muted rounded-lg p-3">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-ink-muted">{r.document_name}</span>
            ...
            <div key={doc.id} className="bg-white rounded-xl shadow-sm p-4 flex items-center justify-between">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-ink-muted">{formatSize(doc.file_size)} · {doc.chunk_count} 个切片{doc.created_at && ` · ${new Date(doc.created_at).toLocaleDateString("zh-CN")}`}</p>
                </div>
              </div>
