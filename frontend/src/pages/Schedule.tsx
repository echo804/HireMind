  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-slate-800">面试日程</h2>
      ...
      <div className="flex items-center justify-between mb-4">
        <button onClick={prevWeek} className="text-sm text-slate-500 hover:text-slate-700">&larr; 上一周</button>
        <span className="text-sm font-medium text-slate-700">
          {weekDates[0].toLocaleDateString("zh-CN")} - {weekDates[6].toLocaleDateString("zh-CN")}
        </span>
        <button onClick={nextWeek} className="text-sm text-slate-500 hover:text-slate-700">下一周 &rarr;</button>
      </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-7 min-h-[300px]">
            {weekDates.map((d, i) => {
              ...
              return (
                <div key={i} className="p-2 border-r border-b border-slate-100 last:border-r-0 min-h-[80px]"
                  onClick={() => openCreate(formatDate(d) + "T10:00")}>
                  {dayEvents.map(e => (
                    <div key={e.id} onClick={e => e.stopPropagation()}
                      className="text-xs p-1.5 mb-1 rounded bg-blue-50 text-blue-700 cursor-pointer hover:bg-blue-100 truncate">
                      {e.candidate_name}
                    </div>
                  ))}
                </div>
      ...
      {!loading && events.length > 0 && (
        <div>
          <h3 className="font-semibold text-slate-800 mb-3">本周安排</h3>
          <div className="grid gap-3">
            {events.sort((a, b) => a.scheduled_at.localeCompare(b.scheduled_at)).map(e => (
              <div key={e.id} className="bg-white rounded-xl p-4 shadow-sm flex items-center justify-between">
                <div>
                  <p className="font-medium text-slate-800">{e.candidate_name}</p>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-xl shadow-xl p-6 w-96" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-slate-800 mb-4">{editEvent ? "编辑日程" : "添加日程"}</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-slate-600 mb-1">候选人姓名 *</label>
              ...
              <div>
                <label className="block text-sm text-slate-600 mb-1">面试类型</label>
              ...
              <div>
                <label className="block text-sm text-slate-600 mb-1">面试时间 *</label>
