...
export default function ResumeList() {
  ...
  const statusBadge = (status: string, score: number | null) => {
    ...
    if (status === "processing") return <span className="text-xs px-2 py-0.5 rounded-full bg-brand-100 text-brand-700 animate-pulse">处理中...</span>;
    return <span className="text-xs px-2 py-0.5 rounded-full bg-surface-muted text-ink">等待中</span>;
  };
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <input
            ...
            className="px-3 py-2 border border-line rounded-lg text-sm w-48 focus:outline-none focus:ring-2 focus:ring-brand-400"
          />
          ...
          {resumes.map((r) => (
            <div key={r.id} className="bg-white rounded-xl p-5 shadow-sm flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <Link to={"/resumes/" + r.id} className="font-medium text-ink hover:text-brand-600">
                    {r.name || r.filename}
                  </Link>
