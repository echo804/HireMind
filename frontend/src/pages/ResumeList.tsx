...
export default function ResumeList() {
  ...
  const loadResumes = useCallback(async () => {
    try {
      ...
      if (body.code === 0) setResumes(body.data || []);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const statusBadge = (status: string, score: number | null) => {
    if (status === "done") {
      const color = score && score >= 80 ? "green" : score && score >= 60 ? "yellow" : "red";
      return <span className={"text-xs px-2 py-0.5 rounded-full bg-" + color + "-100 text-" + color + "-700"}>{score}</span>;
    }
    if (status === "failed") return <span className="text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-700">解析失败</span>;
    return <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-700">处理中</span>;
  };
